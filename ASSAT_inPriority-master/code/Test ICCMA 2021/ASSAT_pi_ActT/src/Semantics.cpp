...
bool Semantics::allsat(SATFormulae sat, vector<Labelling> *labs,
		vector<OrClause> *blocks, ConfigurationStable *blockConf, int numArgs) //求出所有的可行解，放入向量labs中
{
	int retsat = 0; //neither sat nor unsat by default
	vector<vector<int> *> models;//int向量models初始化
	...
	if (blocks == NULL)//retsat是一个Int型的变量
	{
			...
			for (int i = 0; i < af->numArgs(); i++)
			{
				if ((*lastcompfound)->at(i) > 0)
				{
					lab.add_label(af->getArgumentByNumber(i),
							Labelling::lab_in);//争议in
					if (debug)
					{
						cout << af->getArgumentByNumber(i)->getName() << endl;//获取争议的名称
					}
					continue;
				}
				...
				{
					lab.add_label(af->getArgumentByNumber(i),
							Labelling::lab_out);//争议out
					continue;
				}
				if ((*lastcompfound)->at(i + 2 * af->numArgs()) > 0)
				{
					lab.add_label(af->getArgumentByNumber(i),
							Labelling::lab_undec); //争议undec
					continue;
				}
			}
